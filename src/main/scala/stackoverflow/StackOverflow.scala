...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    ...
    questionsPostings.join(answersPostings).groupByKey()
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    scored.map((questionAndScore) => {
      val question = questionAndScore._1
      val tag = question.tags
      val index = firstLangInTag(tag,langs).getOrElse(-1)*langSpread
      (index,questionAndScore._2)
    }).filter(_._1>=0)
      .persist()
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val newMeansPoints = vectors.map(point =>{
      (findClosest(point,centers = means),point)
    }).groupByKey()
      .mapValues(points => averageVectors(points))
      .collect()

    val indexHelper = newMeansPoints.map(_._1)
    //
    val newMeans = means.zipWithIndex.map(vwithIndex=>{
      val indexOfNewMeans = indexHelper.indexOf(vwithIndex._2)
      if(indexOfNewMeans == -1){
        (vwithIndex._2,vwithIndex._1)
      }else{
        newMeansPoints(indexOfNewMeans)
      }
